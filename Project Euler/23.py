...
"""
#  For range(28123), get all the divisors
abundant = []
nonabundant_sums = []
for n in range(1, 28123 + 1):
    ...
    for d in range(1, int(n/2) + 1):
        if n % d == 0:
            divisors.append(d)
#  If sum of divisors > number, add to abundant list
    if sum(divisors) > n:
        abundant.append(n)
print(abundant)
abundant = set(abundant)
#  For each in abundant list, add to other, append to new list if not already there
for n in range(1, 28123 + 1):
    for x in abundant:
        if n - x in abundant:
            break
    else:
        nonabundant_sums.append(n)
#  Sum all numbers in range 28123 that isn't in the list
